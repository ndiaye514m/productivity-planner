...
import { UserService, Visitor } from "./user.service";
...
export class UserFirebaseService implements UserService {
    ...
    create(visitor: Visitor,bearerToken: string): Observable<void> {
    const url = `${this.#USER_COLLECTION_URL}${visitor.id}`;
    const body = {
      fields: {
        name: { stringValue: visitor.name },
        email: { stringValue: visitor.email },
      },
    };
